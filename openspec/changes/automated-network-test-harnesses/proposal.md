...
- `dev-test-runner`: Agent-runnable verification for both logical boot images; scripted full-app interaction with output assertion semantics between steps; selection when multiple logical images exist; headless-first automated pass/fail with actionable failure output.
