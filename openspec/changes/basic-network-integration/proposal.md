...
Prior apply attempts using Linux bridge enslavement disrupted workstation network access when bridge profiles were misconfigured. The workstation runner must attach the guest to the real LAN without taking the host offline.
...
- Require the development-workstation QEMU runner—headless and headful—to attach an emulated NIC directly to the workstation LAN on every invocation, so the guest talks to the same router and DHCP server as the workstation, without disrupting the host's own network connectivity. Use a fixed guest MAC address so repeated runs do not exhaust DHCP leases.
...
- `dev-test-runner`: Mandatory LAN-attached Ethernet on every run mode (headless and headful), stable guest MAC, host connectivity preserved during test setup, and automated assertions for network diagnostic output.
...
- QEMU runner uses ephemeral macvtap (or equivalent LAN passthrough) on a designated workstation wired interface; user-mode or NAT-only networking is not a supported run path.
- Documentation updates for macvtap setup prerequisites (scoped elevated permissions, designated LAN interface, DHCP on the LAN).
